...
nanoJetType = NTupleObjectType("jet",  baseObjectTypes = [ fourVectorType ], variables = [
...
#    NTupleVariable("btagCMVA",  lambda x : x.btag('pfCombinedMVABJetTags'), help="CMVA discriminator"),
    ...
    NTupleVariable("leptonIdx0",   lambda x : x.leptonIdx[0] if hasattr(x,"leptonIdx") and len(x.leptonIdx)>0 else -1, int, mcOnly=False,help="index of the first matching lepton"),
    NTupleVariable("leptonIdx1",   lambda x : x.leptonIdx[1] if hasattr(x,"leptonIdx") and len(x.leptonIdx)>1 else -1, int, mcOnly=False,help="index of the second matching lepton"),
])
