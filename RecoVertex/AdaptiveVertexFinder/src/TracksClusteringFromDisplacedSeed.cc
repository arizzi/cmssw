 ...
 )
{
	...
	for(std::vector<TransientTrack>::const_iterator it = selectedTracks.begin(); it != selectedTracks.end(); it++){
                ...
                if(ip.first && ip.second.value() >= min3DIPValue && ip.second.significance() >= min3DIPSignificance && fabs(it->track().residualX(0)) < maxPixelResidual &&  fabs(it->track().residualY(0)) < maxPixelResidual
