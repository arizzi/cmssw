...
inline reco::TransientTrack buildTT(edm::Handle<std::vector<reco::CandidatePtr> > & tracks, edm::ESHandle<TransientTrackBuilder> &trackbuilder, unsigned int k)
{
	if((*tracks)[k]->bestTrack() == 0) return reco::TransientTrack();
        return trackbuilder->build((*tracks)[k]);
}
